...
class Biblioteca:
    ...
    def prestar_libro(self, isbn: str, cedula: str,
        ...
        if libro is None:
            return f" x Error: No se encontró el libro con ISBN {isbn}"

        estudiante = self._buscar_estudiante(cedula)
        if estudiante is None:
            return f"  x Error: No se encontró el estudiante con cédula {cedula}"

        if not libro.disponible:
            return f" x✗ Error: El libro '{libro.titulo}' no está disponible"
    ...
    def devolver_libro(self, isbn: str, cedula: str) -> str:
        ...
        return "  x Error: No se encontró un préstamo activo con esos datos"
